...
Each backend keeps an exponentially weighted moving average of its request latency in integer milli units, so the arithmetic is fully deterministic and free of floats in the decision path. Every successful request folds its measured latency into the average with a configurable smoothing factor, and callers that have no latency measurement keep using the plain success path, which never touches the average and cannot skew the pool mean. The fold blends in 128 bit arithmetic and saturates at the top of the range, so an adversarial latency sample near `u64::MAX` folds to a saturated average instead of panicking in debug builds or silently wrapping in release, and a later moderate sample still decays the average back down. The ejection line saturates the same way, which keeps the line monotonic: a bigger pool mean never produces a smaller line.
...
- Gate 7 proves the engine stays sane under hostile input and clock edges. A latency sample at the very top of the clock range folds to a saturated EWMA through the full pool path with no panic and no wrap, and the ejection line is checked to saturate monotonically. Time zero arms breakers and outlier ejections correctly and survivors keep serving the outage. A jump to the top of the u64 range promotes the breaker to Half-Open, expires stale ejections, and leaves the pool selectable, with a failure stamped there saturating the next expiry. Weights that share a common divisor, which is where naive smooth weighted implementations drift, still produce exact repeating cycles. Least connections accounting is checked with overlapping in flight requests completed out of order, and an unpaired end saturates at zero instead of wrapping below it.
